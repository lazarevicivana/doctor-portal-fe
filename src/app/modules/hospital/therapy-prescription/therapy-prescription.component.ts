...
import {MedicinePrescriptionService} from "../services/medicine-prescription.service";
...
export class TherapyPrescriptionComponent implements OnInit {
  ...
  public medicinePrescriptionModel = new MedicinePrescriptionModel();
  ...
  constructor(private alert: NgToastService,private bloodPrescriptionService: BloodPrescriptionService,private medicinePrescriptionService: MedicinePrescriptionService, private treatmentReportService: TreatmentReportService, private patientAdmissionService: PatientAdmissionService, private route: ActivatedRoute) { }
  ...
  onSelectingPatient(value: string) {
    ...
    this.treatmentReportService.getTreatmentReport(this.patientAdmission.id).subscribe( res=>{
      ...
      this.medicinePrescriptionModel.treatmentReportId = this.treatmentReportIdDto.id;
  ...
  bloodSubmit(): void {
    ...
    this.bloodPrescriptionService.createPrescription(this.bloodResponse).subscribe(res => {
        this.alert.success({detail: 'Blood prescription added successfully!', duration: 5000})
      },
      error => {
      if(error.status == '404') {
  ...
  onSelectingMedicine(value: string) {
    this.medicinePrescriptionModel.medicineId = value;
  }

  medicineSubmit() : void {
    this.medicinePrescriptionService.createPrescription(this.medicinePrescriptionModel).subscribe(res => {
        this.alert.success({detail: 'Medicine prescription added successfully!', duration: 5000})
      },
      error => {
        if(error.status == '404') {
          this.alert.error({detail: 'There is not enough medicine!', duration: 5000})
        }});
  }
